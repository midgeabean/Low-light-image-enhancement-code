...
def calculate_niqe_for_folder(folder_path):
    # 支持的图片格式
    ...
    for file in image_files:
        ...
        # 如果 niqe() 需要灰度图，可在这里转换
        # im = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)

        score = niqe(im)
        niqe_scores.append(score)
        print(f"{file}: NIQE = {score:.4f}")

    mean_score = np.mean(niqe_scores)
    print("\n📊 NIQE 结果汇总：")
    print(f"共处理 {len(niqe_scores)} 张图片，平均 NIQE = {mean_score:.4f}")

    return niqe_scores, mean_score


if __name__ == "__main__":
    folder_path = '../../output/Non_deep_learning_methods_output/our/Perceptual_Logarithmic_Enhancement_nolvbo'
    #folder_path = '../../dataset/LOLv1/high'
    calculate_niqe_for_folder(folder_path)
